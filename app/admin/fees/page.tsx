...
export default function AdminFeesDashboard() {
  ...
  useEffect(() => {
    async function loadOptions() {
      try {
        ...
        const years: AcademicYear[] = await yearsRes.json();
        const sects: Section[] = await sectionsRes.json();

        setAcademicYears(years);
        setSections(sects);

        const current = years.find((y) => y.isCurrent);
        setAcademicYearId(current ? current.id : years[0]?.id ?? "");
      } catch (err) {
        console.error("Failed to load academic years / sections", err);
      } finally {
  ...
  const fetchStudents = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error || "Failed to load dues");
      setStudents(data);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gray-50 p-8">
      ...
      <div className="mb-8 flex flex-wrap items-end gap-4 rounded-xl bg-white p-6 shadow-sm">
        ...
        <button
          ...
          disabled={!sectionId || !academicYearId}
